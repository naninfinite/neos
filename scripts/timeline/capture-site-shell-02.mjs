...
const outputDir = path.join(root, 'docs', 'timeline', 'assets', '05-site-shell-02');
const homePath = path.join(outputDir, 'site-shell-02-home-hero-stack.png');
const placeholderPath = path.join(outputDir, 'site-shell-02-you-placeholder.png');
...
try {
  ...
  await page.waitForSelector('[aria-label="Site shell"]', { timeout: 4_000 });
  await page.waitForSelector('text=Preparing light glass shell and channel navigation.', {
    state: 'hidden',
    timeout: 5_000,
  });
  await page.waitForSelector('text=Liquid Glass Channel Surface', { timeout: 2_000 });
  ...
  await page.waitForSelector('text=YOU.EXE placeholder', { timeout: 2_000 });
